...
3. [Partition workflows into reasonably distinct sections ] (#partition)
4. [Compose tests for each section ] (#compose_tests)
5. [Produce sufficient code to satisfy each failing test] (#produce_code)
6. [Optimize code for parallel execution of multiple entities] (#optimize)
7. [Define acceptance tests and criteria] (#acceptance)

Here we cover each of the seven steps so you too can build scalable, resilient and dynamic data pipelines to support your applications. 

## <a name="interpret"> Step 1. Interpret legacy data transformation code</a>

Legacy data transformation business logic can exist at multiple levels in an application. Often these are written in-line or as parameterized queries within the application code itself. Sometimes they are hidden by an abstraction layer (e.g. Object-relational Mapping AKA ORM). In this example, we focus on code embedded in T-SQL stored procedures, a common approach when working with relational databases. 

The procedural code lacks coding functionality, particularly in iterative constructs. The T-SQL code is written without tests to identify data errors in stored procedures. Such errors are not generated until run-time, making them hard to debug. 
...
Following is the setup and teardown functions that execute before each test. In the following code snippet, two tests are written to test a functionality that counts the occurrences of a letter in a list of words. The second test captures an exception object when the letter is not found in the dictionary.
...
1. For all data structures that are generated from a section, perform:
  1. Print ‘Size, properties, initial “n” number of rows of the data frames 
  2. Perform initial count checks on sizes and schema types
  3. Identify key or composite key in data frames
  4. Sort the data columns per identified keys
2. Perform ‘diff’ operation on sorted data frame and it’s counterpart procedural output
3. Print ‘matching percentage’ 
	1. If 100% match, print ‘Success test’
	2. If not a 100% match, print the mismatched data points on the screen.
